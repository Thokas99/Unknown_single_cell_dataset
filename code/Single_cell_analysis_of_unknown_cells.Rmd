...
```{r Find best score for tissue, warning=FALSE}
# DB file
...
for (t in tissue) {
  ...
  es.max <- sctype_score(scRNAseqData = XUR[["RNA"]]$scale.data, scaled = TRUE, gs = gs_list$gs_positive, gs2 = gs_list$gs_negative)
...
XUR <- run_sctype(XUR,known_tissue_type="Lung",custom_marker_file =db_ ,name="scType_labels",plot=TRUE)
```